...
unsigned long t2 = 0; // unsigned타입을 사용하는 이유는 millis()함수의 반환타입이 unsigned이기 때문이다
unsigned long t3 = 0; // 버튼이 눌린 횟수 측정하기
unsigned long t4 = 0; // LED

int btn_cnt = 0; // 버튼이 1회 눌려지면 1씩 증가 시킬것
int motion_cnt = 0; // 움직임이 1회 감지되면 1씩 증가할 것
...
void loop() {
  ...
  // now_btn = n번째 상태
  // old_btn = n-1번째 상태
  // 직전 상태가 HIGH였는데 현재 상태가 LOW다 = 하강엣지 == 버튼이 눌러졌음
  if(old_btn == HIGH && now_btn == LOW){
    // 하강엣지
    t3 = millis();
  } else if(old_btn == LOW && now_btn == HIGH) {
    // 상승엣지 누르고 있다가 누름을 해제하는 그 시점
  ...
  // 메인 루프 안에 2가지 이상의 작업이 있을 경우 delay함수는 사용불가능하다.
//  if(millis() - t1 > 1000){
//    t1 = millis();
...
//  }
  ...
  if(millis() - t2 > 3000){
    ...
    // Serial.println("이 문장은 10초 간격으로 출력하고 싶습니다!");
  }
  /*
